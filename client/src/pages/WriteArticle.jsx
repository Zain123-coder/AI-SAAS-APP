  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
     ...
     <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
      ...
      <span onClick={()=>setSelectedLength(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text=== item.text ? 'bg-blue-50 text-blue-700':'text-gray-500 border-gray-500 border-gray-300'} `} key={index}>{item.text}</span>
      ))}
     </div>
    <br />
    ...
    <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
      ...
      {!content ? (<div className='flex-1 flex justify-center items-center'>
        ...
        <div className='mt-3 h-full overflow-y-scroll text-sm text-sm text-slate-600'>
