...
const GenerateImages = () => {
  ...
  return (
     ...
     <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
      {imageStyle.map((item)=>(
      <span onClick={()=>setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedStyle=== item ? 'bg-green-50 text-green-700':'text-gray-500 border-gray-500 border-gray-300'} `} key={item}>{item}</span>
      ))}
     </div>
    ...
    <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r         from-[#4df641] to-[#79eb37] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer  '>
